    ...
    const sendMessage = async (textToSend: string, attachmentsToSend: { data: string; mimeType: string; preview: string }[]) => {
        ...
        try {
            ...
            if (data.reply) {
                ...
                if (isAutoSpeak && tts) {
                    setSpeakingIdx(finalMessages.length - 1);
                ...
                <header className="flex-none h-20 md:h-24 flex justify-between items-center bg-black/80 backdrop-blur-3xl px-6 md:px-12 border-b border-white/10 z-50">
                    ...
                    <div className="flex items-center gap-4">
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl border transition-all text-xs font-black uppercase tracking-widest ${isAutoSpeak ? 'bg-primary/20 border-primary/40 text-primary' : 'bg-white/5 border-white/10 text-white/40'}`}
                        >
                            ...
                            {isAutoSpeak ? "إيقاف الكلام" : "تشغيل تلقائي للصوت"}
                        </button>
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (speakingIdx === idx) {
                                                            ...
                                                            setSpeakingIdx(idx);
                                                            tts?.speak(cleanContent, () => setSpeakingIdx(null));
                                                        }
                                                    }}
                                                    className={`mt-4 w-full flex items-center justify-center gap-2 py-2 rounded-xl transition-all border ${speakingIdx === idx ? 'bg-primary border-primary text-black' : 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10 hover:text-white'}`}
                                                >
                                                    ...
                                                    <span className="text-xs font-black uppercase tracking-widest">{speakingIdx === idx ? "جاري التحدث..." : "استمع للدحيح"}</span>
                                                </button>
                                            )}
                                        </div>
