...
export default function InstallButton() {
    ...
    const handleInstallClick = async () => {
        // We only trigger if the prompt is ready
    ...
    return (
        <div className="relative inline-block">
            <motion.button
                ...
                className={`relative flex items-center gap-3 px-6 py-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-all duration-500 overflow-hidden group ${!installPrompt ? 'opacity-30 cursor-not-allowed grayscale' : 'hover:border-primary/50'}`}
            >
            ...
            <AnimatePresence>
                {showTooltip && installPrompt && (
                    ...
                    >
                        <p className="text-white/80 text-[10px] font-bold leading-relaxed">ثبت التطبيق فوراً بضغطة زر واحدة!</p>
